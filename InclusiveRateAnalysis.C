...
void InclusiveRateAnalysis::GeneratePseudoData( Int_t _n_events, Int_t _n_events_per_lund_file, const char* _dir_for_lund_files, int start_number, Double_t theta_min, Double_t theta_max, Double_t p_min, Double_t p_max )
{
  Int_t eventOpened = 0;
  ...
  /* out lundfile */
  ofstream _out_lund_file ;

  /* produce _n_events simulated data */
  Int_t _iev = 1;
  //  for( Int_t _iev = 0 ; _iev < _n_events + 1 ; _iev++  ){
  while(_iev < _n_events + 1){
    ...
    if (_theta < theta_min || _theta > theta_max  || _energy < p_min || _energy > p_max) {
    }
    else{
      _iev++;
      Int_t _file_number = start_number + TMath::Floor( _iev/_n_events_per_lundfile );
      if(_iev % _n_events_per_lundfile == 0 ) {
	if (eventOpened < _iev && _iev > 0) {
	  cout << _iev <<endl;
	  cout << _file_number <<endl;
	  _out_lund_file.close();
	  cout << "file closed: " << _out_lund_file <<endl;
	  //string oFile = Form("mv l/o.d /w/hallb-scifs17exp/clas12/anya/incEGoutput/fullQ2/m100_1/out%d.dat",_file_number);        
        
        
	  //string string oFile = Form("mv l/o.d /lustre19/expphy/volatile/clas12/valerii/inclusive/eventGen/m100_6/out%d.dat",_file_number);
	  
	 string oFile = Form("mv l/o.d /lustre19/expphy/volatile/clas12/valerii/incEGoutput/m100_4/out%d.dat",_file_number);
	 
	  cout << oFile.c_str() <<endl;
	  system(oFile.c_str());
	}
	eventOpened = _iev;
	cout << "opening file number : " <<  _file_number <<endl;
	_out_lund_file.open( Form( "%s/o.d", _dir_for_lund_files) );
	cout << "file: " << _out_lund_file <<endl;
	_dir_for_lund_files = "l";
	cout << "dir: " << _dir_for_lund_files <<endl;
	cout <<  "file with path: " << Form( "%s/o.d", _dir_for_lund_files) <<endl;
      }
    ...
    // hQ2Gen         -> Fill(   _q2        );
    _out_lund_file << "         1" << "  " <<  "1.0" << "  " <<  "1.0" << "   " <<  "0" << "   " <<  "1" << "   " <<  _xb << "   " <<  _y << "   " <<  _w*_w << "   " <<  _q2 << "   " <<  _nu <<  endl ;

    _out_lund_file << "  " << 1 << "   " <<  "-1.0" << "   " <<  "1" << "   " <<  11 << "     " <<  0 << "     " <<  0 << "   " <<   _px << "   " <<   _py << "   " <<  _pz << "   " <<  _energy << "   " <<  ElectronMass << "   " <<  "0.0000" << "   " <<   "0.0000" << "   " <<   _z <<  endl ;
    }
